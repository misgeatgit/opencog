        ...
        std::list<std::string> args)
{
    ...
    outf  << "Atom(uuid)" << ", Atom(name)" << ", EnteredAt" << ", LastSeenAt" <<
          ", STI" <<  ", DurationInAF"
        ...
        std::list<std::string> args)
{
    ...
    outf << "at_time(sec)" << ", nonNLP_percentage" << ", insect_percentage" <<
         ", poison_percentage" << ", insecticide_percentage" << "\n";
    //for (const auto afs : (_logger_agent)->af_size_stat){
    for (const auto afs : (_logger_agent)->insect_poison_percentage) {
        outf << afs[0] << ", "  <<  afs[1] << ", " << afs[2] << ", " << afs[3] << ", "
             << afs[4] << "\n";
    }
    outf.flush();
    outf.close();
...
std::string InsectPoisonExpModule::do_parse_sent(Request* req,
        std::list<std::string> args)
{
    auto it = args.begin();
    ...
    std::string cmd1 = "(nlp-start-stimulation" + std::to_string(
                           stimulus_amount) + ")";
