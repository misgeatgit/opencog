...
void LoggerAgent::run(void)
{
    ...
    auto find_insecticides = [this](HandleSeq & nonNLP_atoms_AF,
    HandleSeq & insecticide_atoms_AF) {
        for (const auto& name : insecticides) {
            auto it = std::find_if(nonNLP_atoms_AF.begin(),
            nonNLP_atoms_AF.end(), [name](const Handle & h) {
                HandleSeq seq;
                if (h->is_link()) {
    ...
    float insect_af_percentage = static_cast<float>(insect_atoms_AF.size()) /
                                 af_size;
    float poison_af_percentage = static_cast<float>(poison_atoms_AF.size()) /
                                 af_size;
    float nonNLP_af_percentage = static_cast<float>(nonNLP_atoms_AF.size()) /
                                 af_size;
    float insecticide_af_percentage = static_cast<float>
                                      (insecticide_atoms_AF.size()) / af_size;
    if (first_time) {
