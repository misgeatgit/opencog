    ...
    Agent(cs), _atomspace(cs.getAtomSpace())
{
    ...
    std::vector<std::string> preload = {
       ...
       PROJECT_SOURCE_DIR"/opencog/attention/experiment/data/smokes/smokes_db.scm",
       PROJECT_SOURCE_DIR"/opencog/attention/experiment/data/noise.scm",
       PROJECT_SOURCE_DIR"/opencog/attention/experiment/data/smokes/rule_base.scm"
    };

    _bank = &attentionbank(&_atomspace);
    _eval = new SchemeEval(&_atomspace);

    for(auto& path : preload) {
       _eval->eval("(load \""+ path +"\")");    
    }
...
void SmokesDBFCAgent::run()
{
    ...
    while (true) {
        if (first_run) {
            ...
            if (af_set.empty()) {
                std::cerr << "******COULDNT FIND A SMOKES OR FRIENDS "
                    "SOURCE.RETURNING.********\n";
        ...
        for (Handle& h : unique_set) {
            ...
            if (not interesting_atom(h).empty()) {
                save("smokes-fc-result.data",
                     HandleSeq{h},
                     "INTERESTING:" + interesting_atom(h) + "," + std::to_string(
                         cogserver().getCycleCount() - starting_cycle)
                     + "\n");
            }
